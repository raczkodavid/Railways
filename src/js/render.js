...
export class Render {
    ...
    createTileElement(tile, y, x) {
        ...
        img.src = railType != null ? this.railImages[railType] : this.tileImages[tile.type];
    ...
    updateTile(y, x) {
        ...
        const img = this.container.querySelector(`img[data-coordinates="${y},${x}"]`);

        // get the tile and rail type based on the coordinates
        const tile = this.gameState.board[y][x];
        const rail = this.gameState.rails.find(rail => rail.Y === y && rail.X === x);

        // update the image source and rotation
        img.src = rail != null ? this.railImages[rail.type] : this.tileImages[tile.type];
